# --------------------------------------------------
# Module Name:		AddRental
# Purpose:			Adds a rental to a specified student
...
def main(student_id):
	...
	cursor.execute("""UPDATE gym_locker.students SET rentals = rentals + 1 WHERE student_key = "%s" """ % (student_key))

	# Commits changes to database
	db.commit()

	# Retrieves new rentals value
	cursor.execute("""SELECT rentals FROM gym_locker.students WHERE student_key = "%s" """ % (student_key))
